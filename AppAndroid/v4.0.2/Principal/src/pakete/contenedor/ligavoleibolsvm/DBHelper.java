...
public class DBHelper extends SQLiteOpenHelper {
	...
	public static final String TABLAmvpysepteto = "mvpysepteto";
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String sql = String.format("CREATE TABLE %s (id_mvpsepteto INTEGER PRIMARY KEY AUTOINCREMENT, urlsepteto text, urlmvp text, descripcionmvp text)", TABLAmvpysepteto);
		...
		String sql7 = String.format("CREATE TABLE %s (id_opcion INTEGER PRIMARY KEY AUTOINCREMENT, version text, actualizacion text, primeravez text, jornadaactual text)", TABLAopciones);
		...
		db.execSQL(sql);  db.execSQL(sql3);db.execSQL(sql4);
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		db.execSQL(String.format("DROP TABLE IF EXISTS %s", TABLAmvpysepteto));
